...
class exp1_plots:
	...
	def merge_baselines( self ):
		for file_prefix in self.list_of_file_prefix:
			if file_prefix[0]=='M': # in ['G', 'j', 'J', 'Ms', 'T']:
		...
		for file_prefix in self.list_of_file_prefix:
			...
			for baseline in self.list_of_baselines:
				...
				for l,t in zip(lamb_arr, self.param['list_of_time_span']):
					val = res_baseline[baseline][str(l)][reg2(baseline)]['slant']\
	...
	def merge_selected_baseline_with_final_res_FR( self ):
		...
		for file_prefix in self.list_of_file_prefix:
			...
			for baseline in self.list_of_baselines:
				...
				res_arr = []
				for l,t in zip(lamb_arr, self.param['list_of_time_span']):
					val = compute_FR(res_baseline[baseline][str(l)][reg2(baseline)]['slant']\
						['0.8']['prediction'][str(t)], threshold )
					res_arr.append( val )
				data_arr = np.vstack(( data_arr, np.array(res_arr)))
...
def main():
	...
	param = {}
	# huber 
	arr = [0.05,0.1,0.5,0.6, 0.8, 1.0, 1.2]
	...
	obj = exp1_plots( path , list_of_file_prefix, list_of_baselines, param)
