...
class slant:
	...
	# estimate 
	def __init__(self, init_by = None, obj = None , file = None , train = None, test = None, edges = None , data_type = None, param = None , tuning = False , tuning_param = None , list_of_windows = None):
	...
	def estimate_param(self,lamb=None):
		if lamb!=None:
	...
	def predict(self, num_simulation, time_span_input):
		...
		# timestamps = np.concatenate((self.train[:,1], self.test[:,1]))
		# opinions = np.concatenate((self.train[:,2], self.test[:,2]))

		# self.timestamps = timestamps
		# self.users_all = np.concatenate((self.train[:,0],self.test[:,0]))
		# self.list_to_plot=[]
		# print type( self.test.shape[0] )
		# print type( num_simulation)
		...
		#------------------------------------
		# test_int = np.zeros( self.num_test )
		# test_opn = np.zeros( self.num_test )
		#------------------------------------
		# self.time_list=[]
		# self.estimated_int = []
		# self.estimated_opn= []
		# self.estimated_opn_exact=[]
		for msg_index in range(self.test.shape[0]): 
			...
			# predict_test[msg_index,:], msg_set_for_single_instance, curr_int , curr_opn= self.predict_single_instance( self.test[msg_index], num_simulation, time_span_input)  #*****
			...
			# for window_index in range(self.list_of_windows.shape[0]):
			# 	interval = self.list_of_windows[window_index]
			# 	time=self.test[msg_index,1]
			# 	Impirical_int[ window_index, msg_index] = self.get_num_of_msg( time, interval, msg_timestamps )/float(interval)
			# if time_span_input == 0 : 

			# 	test_int[ msg_index ] = np.mean( self.curr_int )
			# 	test_opn[ msg_index ] = np.mean( self.curr_opn )
			# if time_span_input > 0 :  
			# 	test_int[ msg_index ] = np.mean( curr_int )
			# 	test_opn[ msg_index ] = np.mean( curr_opn )
			#------------------------------------------
			# return
		mean_predict_test = np.mean( predict_test, axis = 1 ) 

		#************************************************
		# plt.plot( mean_predict_test , 'r')
		# plt.plot( self.test[:,2] , 'b')
		# plt.show()
		#************************************************
		...
		# results['Impirical_int']=Impirical_int
		# results['estimated_int']=np.concatenate(( train_int, test_int))
		# results['estimated_opn']=np.concatenate(( train_opn, test_opn))
		# if time_span_input == 0 :
			# Impirical_int, Impirical_opn = self.generate_empirical_mean_density_opinion( timestamps, opinions)
		# 	results['Impirical_int']=Impirical_int
		# 	results['Impirical_opn']=Impirical_opn
		# 	# results['Impirical_opn_exact']=np.concatenate((self.train[:,2], self.test[:,2]))
		# 	# results['estimated_opn_exact']=np.array(self.estimated_opn_exact)
		# 	results['estimated_int']=np.array(self.estimated_int)
		# 	results['estimated_opn']=np.array(self.estimated_opn)
		# 	results['original_timestamp'] = timestamps
		# 	results['update_time_list']=np.array(self.time_list)
		# 	results['list_of_windows'] = self.list_of_windows
		# else:
		# 	results['simulated_int']=test_int
		# 	results['simulated_opn']=test_opn
	...
	def simulate_events(self, time_span,mu,alpha,A,B,flag_check_num_msg = False,return_only_opinion_updates= False,var=None,p_exo=None,p_dist=None,noise=None):
