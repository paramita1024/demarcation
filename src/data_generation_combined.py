...
def generate_set_selection_files(file,info,var):
	data=load_data(file)
	res={'rcpk':{}, 'cpk':{}}
	for l in info['set_of_lambda']:
		res['rcpk'][str(l)]=run_RCPK(data,l,info['frac_end'])
		res['cpk'][str(l)]=run_CPK(data,l,info['frac_end'],var)
	return res
...
def main():
	...
	# train_fraction = .9
	# frac_sparse = .6
	generate_graph_flag= True #
	generate_msg_flag=True
	...
	if generate_msg_flag:
		...
		for sim_no in range(num_simul):
			...
			data.generate_msg(plot_file,time_span, option='both_endo_exo',p_exo=.2,var=.1)
			data.split_data(split_frac)
			save(data,output_file+'_'+str(sim_no))
			data.clear_msg()
	...
	if generate_set_selection_files_flag:
		set_selection_res={}
		num_simul=100
		info={'set_of_lambda':[.5,100],'frac_end':0.8}
		result_set_selection_file=path+file_prefix+'_set_selection_result'
		var=0.1
		for sim_no in range(num_simul):
			file=file_common+'_'+str(sim_no)
			set_selection_res[sim_no]=generate_set_selection_files( file,info,var )
			print sim_no
