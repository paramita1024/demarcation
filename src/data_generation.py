...
def generate_result_norm_param(file_graph,file_estim_param,result_file,l_sl,set_of_noise,num_simul,set_of_method, set_of_lambda_cpk):	
	def get_diff(dict_a,dict_b):
		...
		# for l in set_of_lambda:
		# 	plt.plot(estim_param[field_name].flatten()[indices], label=str(l))
		# plt.legend()
		# plt.xlim(200,400)
		# plt.show()
		#----------------------------------delete 
...
def main():
	...
	v=10
	#--
	# file_data= '../result_synthetic_dataset/kroneckerHier512_msg_200000_data.slant.param'
	# data=load_data(file_data)
	...
	if estimation_on_single_msg_stream:
		...
		for max_msg in [500*i for i in set_of_msg]:#75 # [25,50,100,200,400,800]]:
			...
			# file_data= '../result_synthetic_dataset/kroneckerHier512_msg_'+str(max_msg)+'_data.cpk'
			# data=load_data(file_data)
			# print data['0.5'].shape
			# return 
		...
		# check whether results are as desired
		# res=load_data(result_file)
		# plt.plot(range(12),res['10000'][:,0])
		# plt.grid(True)
		# plt.show()
		

		# set_of_lambda=[.5,.7,.9,1,1.5,2]
		# check_slant_single(msg_file, set_of_lambda)
		# return 
	...
	if plot_fraction_flag:
		...
		file_msg_prefix =path+'msg/'+file_prefix +'_data'
		...
		# get_fraction_of_recovery(file_msg_prefix,set_sel_file_prefix,recovery_res_file,lamb,set_of_noise,num_simul,l_cpk)
		plot_fraction_of_recovery(recovery_res_file,plot_file)
	# if plot_fraction_flag_old:cp 
	# 	# set_of_lambda=[.5,.7,1,2,3]
